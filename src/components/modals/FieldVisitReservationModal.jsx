...
function FieldVisitReservationModal({ open, onOpenChange }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log('Event tracked: submit_field_visit_reservation');
    
    // Validate required fields explicitly if HTML5 validation fails or for custom logic
    if (!formData.department || !formData.city || !formData.timeslot || !formData.email || !formData.phone) {
      toast({
        variant: "destructive",
        title: "Erreur",
    ...
    localStorage.setItem('field_visit_booking', JSON.stringify({
      ...formData,
    ...
    toast({
      ...
      description: "Vous allez être redirigé vers la page de remerciement...",
    });
...
export default FieldVisitReservationModal;
